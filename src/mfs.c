...
int main()
{
    ...
    while (1)
    {
        ...
        else if (strcmp(token[0], "ll") == 0)
        {
            printf("Did you mean ls?\n");
        }
...
void popRootDir(FILE *fp, struct fatSpec *specs)
{
    int rootAddress = (specs->BPB_NumFats * specs->BPB_FATSz32 * specs->BPB_BytsPerSec) + (specs->BPB_RsvdSecCnt * specs->BPB_BytsPerSec);
...
int LBAToOffset(int32_t sector, struct fatSpec *specs)
{
    return ((sector - 2) * specs->BPB_BytsPerSec) + (specs->BPB_BytsPerSec * specs->BPB_RsvdSecCnt) + (specs->BPB_NumFats * specs->BPB_FATSz32 * specs->BPB_BytsPerSec);
}
